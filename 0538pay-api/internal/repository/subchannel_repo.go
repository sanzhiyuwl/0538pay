...
type SubChannelPick struct {
	...
	Whitelist string // 主通道商户白名单（自研扩展，选通道时按本商户过滤）
}
...
func (r *SubChannelRepo) FindPickable(uid uint, typeID int) ([]SubChannelPick, error) {
	...
	err := r.db.Table("pay_subchannel AS B").
		Select("B.id AS sub_id, A.id AS channel_id, A.plugin AS plugin, A.rate AS rate, A.pay_min AS pay_min, A.pay_max AS pay_max, B.info AS info, A.apptype AS app_type, A.merchant_whitelist AS whitelist").
