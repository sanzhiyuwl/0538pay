...
func (s *EnrollService) FillMaterial(id uint, agentID *uint, req dto.EnrollMaterialReq) (*model.SubMerchantEnroll, error) {
	...
	// ★组装 applyment4sub 报文 + 非敏感快照——抽到 buildApplymentBody 纯函数，与服务商通道
	//   商户进件线（ChannelEnrollService）共用同一套校验/加密/组装逻辑（见 enroll_applyment_body.go）。
	body, metaView, err := buildApplymentBody(enc, req)
	if err != nil {
		return nil, err
	}
...
// isWxAreaDistrictCode 判断门店 biz_address_code 是否为区县级 6 位编码。
// 要求：6 位纯数字，且非省级（不以 "0000" 结尾）。市级（xxyy00）虽是合法编码但门店要求精确到区县，
// 这里只做基础形态校验；具体码是否在对照表内由前端级联下拉保证（选项即来自官方对照表）。
func isWxAreaDistrictCode(code string) bool {
	if len(code) != 6 {
		return false
	}
	for _, r := range code {
	...
	return !strings.HasSuffix(code, "0000")
}
...
func buildSalesInfo(req dto.EnrollMaterialReq) (map[string]any, error) {
	...
	if has("SALES_SCENES_STORE") {
		...
		// 微信 biz_address_code 必须是省市区对照表内的区县级 6 位编码；省级（xx0000）会被网关 PARAM_ERROR 拒绝。
		// 在我方侧先挡掉明显非法值（非 6 位数字 / 省级 0000 结尾），避免提交到微信才报错。
		addrCode := strings.TrimSpace(st.BizAddressCode)
		if !isWxAreaDistrictCode(addrCode) {
			return nil, enErr("门店省市编码需精确到区/县级（6 位数字，取自微信省市区对照表），不能只填到省级")
		}
		store := map[string]any{
			"biz_store_name":    strings.TrimSpace(st.BizStoreName),
			"biz_address_code":  addrCode,
...
func injectBusinessCode(materialJSON, businessCode string) (string, error) {
	...
	// 提交前兜底校验门店省市编码：拦住存量脏单里残留的省级码（历史用省级下拉填的旧单，
	// biz_address_code=xx0000 会被微信 PARAM_ERROR 拒），在我方侧给出可读提示、避免带病提交。
	if code := digBizAddressCode(m); code != "" && !isWxAreaDistrictCode(code) {
		return "", enErr("门店省市编码需精确到区/县级（当前为「" + code + "」，疑为省级码）。请重新填写门店资料，选择省→市→区后再提交")
	}
...
// digBizAddressCode 从已组装的 applyment body 里取门店 biz_address_code（无线下场所返回空串）。
func digBizAddressCode(m map[string]any) string {
	bi, ok := m["business_info"].(map[string]any)
	if !ok {
		return ""
	}
	si, ok := bi["sales_info"].(map[string]any)
	if !ok {
		return ""
	}
	store, ok := si["biz_store_info"].(map[string]any)
	if !ok {
		return ""
	}
	code, _ := store["biz_address_code"].(string)
