...
import { Search, Eye, RefreshCw, MessageSquare, History, ImagePlus, X, Link2, Send } from 'lucide-vue-next'
...
import {
  ...
  adminReplyComplaint,
  adminReplyComplaintImmediate,
  adminCompleteComplaint,
  adminUpdateComplaintRefund,
  adminUploadComplaintImage,
  adminGetComplaintNotifyURL,
  adminSetComplaintNotifyURL,
  adminDeleteComplaintNotifyURL,
  adminReconcileComplaints,
...
// —— 回调地址自管理 ——
const notifyURL = reactive<NotifyURLState>({ registered: '', local: '' })
...
async function loadNotifyURL() {
  try {
    const res = await adminGetComplaintNotifyURL()
    Object.assign(notifyURL, res)
    notifyInput.value = res.local || res.registered || ''
  } catch {
    /* 凭证未配齐时静默，设置弹窗里再提示 */
  }
}
async function saveNotifyURL() {
  const url = notifyInput.value.trim()
  if (!url.startsWith('https://')) {
    toast.error('回调地址必须是 https:// 开头的公网地址')
    return
  }
  notifyBusy.value = true
  try {
    const res = await adminSetComplaintNotifyURL(url)
    Object.assign(notifyURL, res)
    toast.success('回调地址已设置')
...
async function removeNotifyURL() {
  if (!(await confirm('删除后微信将不再向本平台推送投诉通知，确认删除？', { title: '删除回调地址', danger: true }))) return
  notifyBusy.value = true
  try {
    await adminDeleteComplaintNotifyURL()
    notifyURL.registered = ''
    toast.success('已删除微信侧回调地址')
  } catch (e) {
...
async function reconcile() {
  ...
  try {
    const res = await adminReconcileComplaints()
    toast.success(`对账完成，同步 ${res.synced} 单`)
    await load()
  } catch (e) {
    toast.error(e instanceof ApiError ? e.message : '对账失败')
  } finally {
...
async function openDetail(row: ComplaintListItem) {
  ...
  resetReply()
  try {
    detail.value = await adminComplaintDetail(row.id)
    loadHistory(row.id)
  } catch (e) {
...
async function loadHistory(id: number) {
  ...
  try {
    const res = await adminComplaintHistory(id)
...
async function syncDetail() {
  ...
  syncing.value = true
  try {
    detail.value = await adminSyncComplaint(detail.value.complaint.id)
...
async function onFileChange(e: Event) {
  ...
  try {
    const res = await adminUploadComplaintImage(file)
...
async function submitReply() {
  ...
  try {
    ...
    if (canImmediate.value) {
      await adminReplyComplaintImmediate(id, { content, images: replyImages.value })
    } else {
      await adminReplyComplaint(id, { content, images: replyImages.value })
    }
    toast.success('已回复用户')
    resetReply()
    detail.value = await adminComplaintDetail(id)
    loadHistory(id)
    load()
  } catch (e) {
...
async function complete() {
  ...
  try {
    const id = detail.value.complaint.id
    await adminCompleteComplaint(id)
...
async function submitRefund() {
  ...
  try {
    const id = detail.value.complaint.id
    await adminUpdateComplaintRefund(id, {
...
<template>
  <div class="space-y-2.5">
    ...
    <Panel
      title="消费者投诉"
      subtitle="微信支付消费者投诉2.0：投诉回调只推服务商回调地址，平台按被诉子商户反查商户后统一受理（守不走二清红线，仅处理本平台名下已进件成功子商户的投诉）"
    >
      <template #actions>
        <div class="flex items-center gap-2">
          <Button size="sm" variant="outline" @click="notifyOpen = true"><Link2 class="size-3.5" />回调地址</Button>
          <Button size="sm" variant="outline" :disabled="reconciling" @click="reconcile">
            <RefreshCw class="size-3.5" :class="{ 'animate-spin': reconciling }" />对账
          </Button>
        </div>
      </template>
      <div class="flex flex-wrap items-center gap-x-10 gap-y-4">
        ...
        <div v-if="notifyURL.registered" class="ml-auto min-w-0 max-w-[42%]">
          <div class="text-[13px] text-muted-foreground">微信已注册回调</div>
          <div class="mt-1 truncate text-xs tabular-nums" :title="notifyURL.registered">{{ notifyURL.registered }}</div>
        </div>
      </div>
    </Panel>

    <!-- 列表 -->
    <Panel title="投诉工单" :subtitle="`共 ${total} 单`">
      ...
      <div class="overflow-x-auto">
        <table class="tbl w-full">
          <thead>
            <tr>
              <th class="w-[16%]">投诉单号</th>
              <th class="w-[14%]">商户 / 子商户号</th>
              <th class="w-[10%]">问题类型</th>
              <th class="w-[22%]">投诉内容</th>
              <th class="w-[9%] col-right">申诉退款</th>
              <th class="w-[9%]">状态</th>
          ...
          <tbody>
            <tr v-for="row in list" :key="row.id">
              ...
              <td>
                <div class="tabular-nums">{{ row.merchant_id || '—' }}</div>
                <div class="mt-0.5 truncate text-xs text-muted-foreground" :title="row.complainted_mchid">{{ row.complainted_mchid || '—' }}</div>
              </td>
    ...
    <Drawer v-model="drawerOpen" title="投诉详情" :subtitle="detail?.complaint.complaint_id" width="max-w-2xl">
      ...
      <div v-else-if="detail" class="space-y-4">
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-1 bg-muted/40 px-4 py-3 text-sm">
          ...
          <span class="min-w-0 truncate font-medium" :title="detail.complaint.merchant_name">{{ detail.complaint.merchant_name || '未反查到本地商户' }}</span>
          <span class="shrink-0 text-xs text-muted-foreground">商户 <span class="text-foreground tabular-nums">{{ detail.complaint.merchant_id || '—' }}</span></span>
        ...
        <div>
          ...
          <p v-else-if="!historyList.length" class="py-4 text-center text-xs dim">暂无协商历史（或服务商凭证未配齐无法现查）</p>
    ...
    <!-- 回调地址设置弹窗 -->
    <Modal v-model="notifyOpen" title="投诉回调地址">
      <div class="space-y-3">
        <p class="text-xs leading-relaxed text-muted-foreground">
          微信投诉通知会推送到此地址。一个服务商商户号只能设置一个回调，须为 https 公网地址。设置后微信回调 → 平台验签解密 → 落库分发。
        </p>
        <div>
          <label class="mb-1 block text-xs text-muted-foreground">回调地址</label>
          <input v-model="notifyInput" class="field-input w-full" placeholder="https://your-domain/api/notify/wx-complaint" />
        </div>
        <div v-if="notifyURL.registered" class="bg-muted/40 px-3 py-2 text-xs">
          <span class="text-muted-foreground">微信侧当前：</span><span class="tabular-nums">{{ notifyURL.registered }}</span>
        </div>
      </div>
      <template #footer>
        <Button v-if="notifyURL.registered" variant="outline" :disabled="notifyBusy" @click="removeNotifyURL">删除</Button>
        <Button :disabled="notifyBusy" @click="saveNotifyURL">保存</Button>
      </template>
    </Modal>
