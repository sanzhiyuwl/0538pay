...
async function load() {
  loading.value = true
  try {
    const res = await fetchConsoleOpLogs(buildParams())
    rows.value = res.list
    total.value = res.total
  } catch (e) {
    toast.error(e instanceof ApiError ? e.message : '加载管理日志失败')
    rows.value = []
    total.value = 0
  } finally {
    loading.value = false
...
onMounted(async () => {
  try {
    const res = await fetchConsoleOpActionOptions()
...
async function exportList() {
  ...
  try {
    await exportConsoleOpLogs({ ...buildParams(), page: undefined, pageSize: undefined })
    toast.success('已开始下载管理日志 CSV')
  } catch (e) {
...
<template>
  <div class="space-y-2.5">
    <Panel title="管理日志" subtitle="记录平台运营在控制台 /console 的写操作（管代理 / 改权限 / 发名额 / 建单 / 退款 / 改结算等），供审计追溯">
      <div class="filter-bar">
        ...
        <div class="filter-item">
          ...
          <input v-model="filters.keyword" placeholder="操作者 / 对象" class="field-input w-36" @keyup.enter="search" />
        </div>
        <div class="filter-item">
    ...
    <Panel title="管理日志列表" :subtitle="`共 ${total} 条`">
      <div class="overflow-x-auto">
        <table class="tbl w-full table-fixed">
          <thead>
            <tr>
              <th class="w-[6%]">ID</th>
              <th class="w-[16%]">操作者</th>
              <th class="w-[15%]">操作</th>
              <th class="w-[9%]">级别</th>
              <th class="w-[22%]">对象</th>
              <th class="w-[8%]">结果</th>
          ...
          <tbody>
            ...
            <tr v-else-if="!rows.length">
              <td colspan="8" class="py-10 text-center dim">没有符合条件的管理日志</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <Modal v-model="detailOpen" title="管理日志详情" width="max-w-lg">
      <div v-if="detailRow" class="space-y-3 text-sm">
        <div class="grid grid-cols-2 gap-x-4 gap-y-2.5">
          <div><span class="dim">操作者：</span>{{ detailRow.operator || `#${detailRow.uid}` }}</div>
