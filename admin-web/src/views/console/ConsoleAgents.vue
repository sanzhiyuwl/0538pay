...
import { Panel, Button, Switch, Drawer, Pagination } from '@/components/ui'
import {
  fetchAgents,
  createAgent,
  updateAgent,
...
async function load() {
  ...
  try {
    ...
    alert(e instanceof ApiError ? e.message : '加载代理失败')
  } finally {
    loading.value = false
  }
}
...
async function remove(a: Agent) {
  if (!confirm(`确定删除代理「${a.name}」？删除后其名下数据不再展示。`)) return
  try {
    ...
    <Panel title="代理管理" :subtitle="`共 ${total} 个代理`">
      ...
      <div class="filter-bar">
        ...
        <div class="filter-item">
          ...
          <select v-model="filters.status" class="field-input w-28">
