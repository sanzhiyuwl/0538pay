...
import {
  ...
  announces,
  settleTrend,
} from '@/lib/mock/merchant/dashboard'
...
const cards = computed(() => [
  { label: '商户当前余额', value: formatMoney(m.money), money: true, icon: WalletIcon, color: 'text-primary' },
  { label: '已结算余额', value: formatMoney(m.settleMoney), money: true, icon: CheckSquare, color: 'text-success' },
  { label: '订单总数', value: String(m.orders), money: false, icon: AreaChart, color: 'text-foreground' },
  { label: '今日订单', value: String(m.ordersToday), money: false, icon: ShoppingCart, color: 'text-warning' },
])

const statusMeta = computed(() => merchantStatusMeta[m.status])

// 结算趋势图
const settleSeries = computed(() => [
  { name: '结算金额', color: '#4b7bec', data: settleTrend.data },
])
...
const alerts = computed(() => {
  ...
  if (dashboardAlerts.needCert) list.push({ key: 'cert', text: '您的账户尚未完成实名认证，部分功能受限，请尽快认证。' })
  if (dashboardAlerts.noSecurity) list.push({ key: 'security', text: '您还未绑定密保手机/邮箱，账户存在安全风险，建议前往账户设置绑定。' })
  if (dashboardAlerts.noLoginPwd) list.push({ key: 'pwd', text: '您还未设置登录密码，仅能使用密钥登录，建议设置登录密码。' })
          ...
          <div class="mt-5 grid grid-cols-2 gap-4 border-t border-border/60 pt-5">
            <div>
              ...
              <div class="mt-1.5 text-xl font-normal tabular-nums text-success">
                <span class="text-sm opacity-70">¥</span>{{ formatMoney(m.todayIncome) }}
              </div>
            </div>
            <div>
              <div class="text-[13px] text-muted-foreground">昨日收入</div>
              <div class="mt-1.5 text-xl font-normal tabular-nums">
                <span class="text-sm opacity-70">¥</span>{{ formatMoney(m.yesterdayIncome) }}
              </div>
            </div>
          </div>
        </div>
